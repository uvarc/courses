...
Read more about basic usage of jq.
...
Create a text file with the path to bash in the first line, with a shebang #! prepended.
Add shell commands below that, in the order you need and expect them to run.
Assume the script is running from its current location, so directory paths need to be explicit.
A simple example:
